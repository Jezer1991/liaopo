...
class AddOpcion extends React.Component {
    ...
    componentWillMount() {
        setTimeout(() => {
            fetch(`${process.env.REACT_APP_API}opcion/${this.state.id_pregunta}`)
                .then(data => {
                    return data.json();
                }).then(data => {
                    this.setState({
            ...
            fetch(`${process.env.REACT_APP_API}pregunta/${this.state.id_pregunta}`)
                .then(data => {
                    return data.json();
                }).then(data => {
                    this.setState({
    ...
    delete(e, id) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                axios.delete(`${process.env.REACT_APP_API}delete/opcion/${id}`)
                    .then(data => {
                        if (data.status === 200) {
                            Swal.fire({
                                title: 'Borrado',
    ...
    render() {
        return (
            <React.Fragment>
                ...
                <form action={`${process.env.REACT_APP_API}save/opcion`} method="post">
